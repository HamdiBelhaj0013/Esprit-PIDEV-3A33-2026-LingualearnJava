...
public class FrontProgressService {
    ...
    public void completeLesson(int userId, int lessonId, int xpReward) throws Exception {
        ...
        String checkSql = "SELECT is_completed FROM user_lesson_status WHERE user_id = ? AND lesson_id = ?";
        try (PreparedStatement ps = cnx.prepareStatement(checkSql)) {
            ps.setInt(1, userId);
            ps.setInt(2, lessonId);
            ResultSet rs = ps.executeQuery();
            if (rs.next() && rs.getBoolean("is_completed")) {
                return; // already done — do not add XP again
            }
        }

        // Mark as completed
        String markSql = """
            INSERT INTO user_lesson_status (user_id, lesson_id, is_completed, best_quiz_score, last_quiz_score, completed_at)
            VALUES (?, ?, true, 0, 0, NOW())
            ON DUPLICATE KEY UPDATE is_completed = true, completed_at = NOW()
            """;
        try (PreparedStatement ps = cnx.prepareStatement(markSql)) {
            ps.setInt(1, userId);
            ps.setInt(2, lessonId);
            ps.executeUpdate();
        }

        // Add XP to learning_stats
        // We also update last_study_session to NOW()
        String statsSql = """
    ...
    public int getEarnedXpForCourse(int userId, int courseId) {
        String sql = """
            ...
            WHERE uls.user_id = ? AND l.course_id = ? AND uls.is_completed = true
            """;
    ...
    public int getCompletedLessonsCount(int userId) {
        try (PreparedStatement ps = cnx.prepareStatement(
                "SELECT COUNT(*) FROM user_lesson_status WHERE user_id = ? AND is_completed = true")) {
            ps.setInt(1, userId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getInt(1);
        } catch (Exception e) { e.printStackTrace(); }
    ...
    public List<Lesson> getRecentCompletedLessons(int userId, int limit) {
        ...
        String sql = """
            ...
            FROM user_lesson_status uls
            JOIN lesson l ON uls.lesson_id = l.id
            WHERE uls.user_id = ? AND uls.is_completed = true
