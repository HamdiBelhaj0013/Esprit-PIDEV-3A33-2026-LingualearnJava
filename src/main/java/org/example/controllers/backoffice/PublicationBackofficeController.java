...
public class PublicationBackofficeController {
    ...
    @FXML
    public void openCommentairesPage() {
        Publication selected = publicationTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showWarning("Selectionne une publication pour gerer ses commentaires.");
            return;
        }

        try {
            BackofficeContext.setSelectedPublicationId(selected.getId());
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/backoffice/fxml/commentaire_manager.fxml"));
            Parent root = loader.load();
            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.initOwner(publicationTable.getScene().getWindow());
            stage.setTitle("Commentaires de la publication #" + selected.getId());
            stage.setScene(new Scene(root));
            stage.showAndWait();
            refresh();
        } catch (Exception e) {
