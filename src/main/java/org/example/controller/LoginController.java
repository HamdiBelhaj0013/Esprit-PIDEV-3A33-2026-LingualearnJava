...
public class LoginController {
    ...
    private void handleLogin() {
        ...
        org.example.entities.User user = userService.getUserByEmail(email);

        if (user != null) {
            // BYPASS TOTAL POUR LE DÉVELOPPEMENT
            // Si l'utilisateur existe, on le connecte directement
            System.out.println(">>> LOGIN SUCCESS (Bypass) for: " + email);
            
            org.example.util.UserSession.currentUser = user;
            boolean isAdmin = user.getRoles() != null && user.getRoles().contains("ROLE_ADMIN");
            org.example.util.UserSession.isAdmin = isAdmin;

            if (isAdmin) {
                navigateTo("/fxml/QuizView.fxml", "Admin Dashboard - LinguaLearn");
            } else {
                navigateTo("/fxml/QuizView.fxml", "Client Portal - LinguaLearn");
            }
        } else {
            System.out.println(">>> User NOT FOUND in database for email: " + email);
            showError("Utilisateur non trouvé. Veuillez vérifier l'email.");
        }
    }
