...
public class DashboardController {
    ...
    // The initial KPI+charts content saved on load
    private Node dashboardContent;

    @FXML
    public void initialize() {
        Platform.runLater(() -> {
            // Save the initial dashboard content (KPI + charts)
    ...
    public void showDashboard() {
        // Restore KPI+charts into contentArea
    ...
    public void showPublications() {
        // Load publication_manager inside contentArea (like AdminMain loads views)
        loadIntoContent("/fxml/admin/forum/fxml/publication_manager.fxml");
    }
    ...
    public void goToAdminDashboard() {
        try {
            BackofficeNav.navigate(sidebar, "/fxml/admin/AdminMain.fxml", "LinguaLearn - Admin");
        } catch (IOException e) {
    ...
    private void updateCharts() {
        try {
            ...
            if (!pubs.isEmpty()) {
                ...
                pubs.stream().sorted((a, b) -> Integer.compare(b.getLikes(), a.getLikes())).limit(5)
                        .forEach(p -> series.getData().add(new XYChart.Data<>(
                                p.getTitrePub() != null && p.getTitrePub().length() > 15
                                        ? p.getTitrePub().substring(0, 15) + "..." : p.getTitrePub(),
                                p.getLikes())));
                topLikedChart.getData().setAll(series);
            }
            ...
            if (!coms.isEmpty()) {
                ...
                comCount.entrySet().stream().sorted((a, b) -> Long.compare(b.getValue(), a.getValue())).limit(5)
                        .forEach(e -> series.getData().add(new XYChart.Data<>("Pub #" + e.getKey(), e.getValue())));
                commentsPerPubChart.getData().setAll(series);
            }

            if (!pubs.isEmpty()) {
                XYChart.Series<String, Number> series = new XYChart.Series<>();
                pubs.stream().filter(p -> p.getReportPub() > 0)
                        .sorted((a, b) -> Integer.compare(b.getReportPub(), a.getReportPub())).limit(5)
                        .forEach(p -> series.getData().add(new XYChart.Data<>(
                                p.getTitrePub() != null && p.getTitrePub().length() > 15
                                        ? p.getTitrePub().substring(0, 15) + "..." : p.getTitrePub(),
                                p.getReportPub())));
                mostReportedChart.getData().clear();
                if (!series.getData().isEmpty()) mostReportedChart.getData().add(series);
            }
        } catch (Exception e) {
    ...
    public void exportPdf() {
        ...
        Window window = sidebar.getScene() != null ? sidebar.getScene().getWindow() : null;
        ...
        try {
            ...
            Table table = new Table(UnitValue.createPercentArray(new float[]{1, 3, 2, 1, 1})).useAllAvailableWidth();
    ...
    private void showInfo(String header, String content) {
        ...
        a.setTitle("Info"); a.setHeaderText(header); a.setContentText(content); a.showAndWait();
    }
}
